...
def fetch_via_selenium_manual(url: str) -> (BeautifulSoup | None):
    ...
    try:
        ...
        print("Page appears blocked or requires verification.")
        print("Please solve the captcha/verification in the opened browser.")
        print("When done, press Enter here to continue, or type 's' then Enter to skip this URL.")
        try:
...
def main():
    try:
        df = pd.read_csv("alonhadat_listings.csv")
    except Exception as e:
    ...
    while not df.empty:
        ...
        details_path = "alonhadat_details.csv"
