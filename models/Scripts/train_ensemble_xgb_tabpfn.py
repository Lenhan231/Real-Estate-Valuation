"""
TabPFN & XGBoost Training Pipeline with Weights & Biases Tracking
=========================================================
Trains an ensemble regressor (XGB + TabPFN) to predict house prices 
...
def plot_feature_importance(model, feature_names, save_path):
    # ponytail: Using XGBoost for feature importances as TabPFN may not support it easily
...
def main():
    ...
    print(f"============================================================")
    print(f"  Ensemble Training (TabPFN+XGBoost) — Dataset: {dataset_label}")
    print(f"============================================================\n")
    ...
    # ------------------------------------------------------------------
    # 4. Train TabPFN & XGBoost (6-Bucket Ensemble Models)
    # ------------------------------------------------------------------
    print("[4/5] Training Ensembles (XGBoost + TabPFN)...")
    try:
        from tabpfn import TabPFNRegressor
    except ImportError:
        print("  TabPFN not installed. Using a dummy or breaking.")
        TabPFNRegressor = None
    try:
        from xgboost import XGBRegressor
    except ImportError:
        print("  XGBoost not installed. Falling back to TabPFN only.")
        XGBRegressor = None
    ...
    tabpfn_params = {} # ponytail: default params
    ...
    for price_bin in BIN_LABELS:
        for ptype in ['property_type_nha_mat_tien', 'property_type_nha_trong_hem']:
            ...
            if TabPFNRegressor is not None:
                model_tabpfn = TabPFNRegressor(**tabpfn_params)
                # ponytail: TabPFN fits on max 10000 samples typically, skipped eval_set
                model_tabpfn.fit(X_tr_b, y_tr_b)
            
            # Train XGBoost
            model_xgb = None
            if XGBRegressor is not None:
                model_xgb = XGBRegressor(**xgb_params)
                model_xgb.fit(X_tr_b, y_tr_b, eval_set=[(X_te_b, y_te_b)] if len(X_te_b) > 0 else None, verbose=False)
            
            if len(X_te_b) > 0:
                y_log_pred_b = 0
                count = 0
                if model_tabpfn is not None:
                    y_log_pred_b += model_tabpfn.predict(X_te_b)
                    count += 1
                if model_xgb is not None:
                    y_log_pred_b += model_xgb.predict(X_te_b)
                    count += 1
                if count > 0:
                    y_log_pred_b /= count
                else:
                    y_log_pred_b = np.zeros(len(X_te_b))
            ...
            if model_xgb is not None:
                models[f"{price_bin}_{ptype}_xgb"] = model_xgb
            
            if model_tabpfn is not None:
                tabpfn_path = MODEL_DIR / f"tabpfn_{price_bin}_{ptype.replace('property_type_', '')}.pkl"
                joblib.dump(model_tabpfn, tabpfn_path)
            if model_xgb is not None:
                xgb_path = MODEL_DIR / f"xgb_{price_bin}_{ptype.replace('property_type_', '')}.pkl"
    ...
    print(f"\n{'='*60}")
    print(f"  Results — {dataset_label} (6-Bucket Global, TabPFN+XGBoost)")
    print(f"{'='*60}")
    ...
    fi_path = None
    if "mid_property_type_nha_trong_hem_xgb" in models:
        fi_path = plot_feature_importance(models["mid_property_type_nha_trong_hem_xgb"], feature_names, plot_dir / f"feature_importance_xgb_{dataset_label}_mid_alley.png")
    
    pva_path = plot_pred_vs_actual(global_y_test, global_y_pred, plot_dir / f"pred_vs_actual_{dataset_label}_global_tabpfn_xgb.png")
    print(f"  Plots saved to {plot_dir}")

    print("  Logging to W&B...")
    run = wandb.init(
        project=WANDB_PROJECT,
        name=f"ensemble-xgb-tabpfn-{dataset_label}-{data_source}",
        config={
    ...
    if fi_path:
        wandb.log({"feature_importance_mid_alley_xgb": wandb.Image(str(fi_path))})
