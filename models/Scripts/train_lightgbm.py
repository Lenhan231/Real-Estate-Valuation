"""
LightGBM Training Pipeline with Weights & Biases Tracking
=========================================================
Trains a LightGBM regressor to predict house prices 
...
def main():
    ...
    print(f"  Training single LightGBM model (Train: {len(X_train)}, Test: {len(X_test)})...")
    
    model = LGBMRegressor(**lgb_params)
    eval_set = [(X_test, y_log_test)] if len(X_test) > 0 else None
    
    model.fit(X_train, y_log_train, eval_set=eval_set, callbacks=[])
    ...
    model_path = MODEL_DIR / f"lgbm_{dataset_label}_{data_source}.pkl"
    ...
    if True:
        fi_path = plot_feature_importance(model, feature_names, plot_dir / f"feature_importance_lgbm_{dataset_label}.png")
    ...
    run = wandb.init(
        ...
        name=f"lgbm-{dataset_label}-{data_source}",
        config={
