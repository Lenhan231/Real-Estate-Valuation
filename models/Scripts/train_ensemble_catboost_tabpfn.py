"""
TabPFN & CatBoost Training Pipeline with Weights & Biases Tracking
=========================================================
Trains an ensemble regressor (CatBoost + TabPFN) to predict house prices 
...
def plot_feature_importance(model, feature_names, save_path):
    # ponytail: Using CatBoost for feature importances as TabPFN may not support it easily
...
def main():
    ...
    print(f"============================================================")
    print(f"  Ensemble Training (TabPFN+CatBoost) — Dataset: {dataset_label}")
    print(f"============================================================\n")
    ...
    # ------------------------------------------------------------------
    # 4. Train TabPFN & CatBoost (6-Bucket Ensemble Models)
    # ------------------------------------------------------------------
    print("[4/5] Training Ensembles (CatBoost + TabPFN)...")
    try:
    ...
    try:
        ...
        print("  CatBoost not installed. Falling back to TabPFN only.")
    ...
    for price_bin in BIN_LABELS:
        for ptype in ['property_type_nha_mat_tien', 'property_type_nha_trong_hem']:
            ...
            if CatBoostRegressor is not None:
                ...
                model_cb.fit(X_tr_b, y_tr_b, eval_set=[(X_te_b, y_te_b)] if len(X_te_b) > 0 else None, verbose=False)
            ...
            if len(X_te_b) > 0:
                ...
                if model_cb is not None:
                    y_log_pred_b += model_cb.predict(X_te_b)
                    count += 1
                if count > 0:
                    y_log_pred_b /= count
                else:
            ...
            if model_cb is not None:
                models[f"{price_bin}_{ptype}_cb"] = model_cb
    ...
    print(f"\n{'='*60}")
    print(f"  Results — {dataset_label} (6-Bucket Global, TabPFN+CatBoost)")
    print(f"{'='*60}")
    ...
    fi_path = None
    if "mid_property_type_nha_trong_hem_cb" in models:
        fi_path = plot_feature_importance(models["mid_property_type_nha_trong_hem_cb"], feature_names, plot_dir / f"feature_importance_cb_{dataset_label}_mid_alley.png")
    
    pva_path = plot_pred_vs_actual(global_y_test, global_y_pred, plot_dir / f"pred_vs_actual_{dataset_label}_global_tabpfn_cb.png")
    print(f"  Plots saved to {plot_dir}")

    print("  Logging to W&B...")
    run = wandb.init(
        project=WANDB_PROJECT,
        name=f"ensemble-cb-tabpfn-{dataset_label}-{data_source}",
        config={
    ...
    if fi_path:
        wandb.log({"feature_importance_mid_alley_cb": wandb.Image(str(fi_path))})
