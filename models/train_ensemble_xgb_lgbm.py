"""
LGBM & XGBoost Training Pipeline with Weights & Biases Tracking
=========================================================
Trains an ensemble regressor (XGB + LGBM) to predict house prices 
...
def main():
    ...
    # ------------------------------------------------------------------
    print("[4/5] Training Ensembles (XGBoost + LGBM)...")
    from lightgbm import LGBMRegressor
    try:
        from xgboost import XGBRegressor
    except ImportError:
        print("  XGBoost not installed. Falling back to LGBM only.")
        XGBRegressor = None
        ...
        for ptype in ['property_type_nha_mat_tien', 'property_type_nha_trong_hem']:
            ...
            if len(X_te_b) > 0:
                ...
                if model_xgb is not None:
                    y_log_pred_xgb = model_xgb.predict(X_te_b)
                    y_log_pred_b = (y_log_pred_lgb + y_log_pred_xgb) / 2.0
                else:
            ...
            if model_xgb is not None:
                xgb_path = MODEL_DIR / f"cb_{price_bin}_{ptype.replace('property_type_', '')}.pkl"
    ...
    run = wandb.init(
        ...
        name=f"ensemble-xgb-lgbm-{dataset_label}-{data_source}",
        config={
