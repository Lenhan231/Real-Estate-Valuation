...
The system is currently deployed as a local demonstration environment rather than a hosted production service. At this stage, the repository does not include a Dockerfile, CI/CD pipeline, or cloud deployment configuration.

The deployment consists of:

* **Application layer:** The Streamlit application is launched with streamlit run app/app.py for the valuation interface and dashboard, while the Flask API is started with python app/api\_simple.py. Both are designed for local or single-host execution.  
* **Configuration management:** Credentials (e.g., Supabase and geocoding keys) are stored in a .env file, loaded with python-dotenv, and excluded from version control through .gitignore.  
* **Data layer:** Processed datasets are synchronized to **Supabase (PostgreSQL)** via pipeline/supabase\_handler.py, providing a persistent and shared backend for data storage.  
* **Model artifacts:** Trained models are stored as local .pkl and .joblib files under models/ and models/saved\_models/, and are loaded directly into memory when the application starts. No model registry or containerized serving infrastructure is currently used.

**Future work:** Containerize the Streamlit application and/or Flask API with Docker and deploy to a lightweight cloud platform (e.g., Streamlit Community Cloud, Render, or Railway) or a virtual machine using Gunicorn, while continuing to use the existing Supabase instance as the shared data backend.

## **4\. Scalability and Maintenance** {#4.-scalability-and-maintenance}

* **Ingestion scalability:** The geocoding and POI lookup pipeline uses a two-tier cache (in-memory and persistent CSV in data/localities.csv). Cached results reduce repeated API calls to external services such as Nominatim and Overpass, improving data collection efficiency.  
* **Inference scalability:**The models support efficient single-property inference, although latency and concurrent throughput have not been formally benchmarked. Models are cached in memory using st.cache\_resource, eliminating reload overhead during a session.  
* **Model maintenance and versioning:**Model retraining is currently manual through scripts/train\_ensemble.py, with experiments tracked in Weights & Biases and summarized in LATEST\_UPDATE.md. There is no automated retraining schedule or model registry, making this an important area for future improvement as market conditions evolve.  
* **Codebase maintenance:**Parallel development resulted in two serving paths (Streamlit and Flask API) and multiple historical training scripts (train.py, train\_ensemble.py, and train\_tabpfn.py). Consolidating these into the final 6-bucket production architecture and archiving obsolete components is the primary maintenance priority.
